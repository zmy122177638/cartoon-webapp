<template>
    <div class="payAdministration_container" ref="scrollview">
        <div class="payAdministration_wrap" v-if="payAutoData.length != 0">
            <ul class="pay_list" >
                <li class="pay_item" v-for="item in payAutoData" :key="item.cid">
                    <figure><img src="" v-lazy="item.img_url" alt="" class="pay_item_img"></figure>
                    <div class="pay_item_c">
                        <h3>{{item.name}}</h3>
                        <p>第{{item.sort_number}}话</p>
                        <p>￥{{item.coin}}</p>
                        <p>{{item.createtime}}</p>
                    </div>
                    <div class="payautoBtn" @click="setPayAutoEvent(item)">取消购买</div>
                </li>
            </ul>
            <p class="load_promt" v-if="loadShow">没有更多了~</p>
        </div>
        <fail-view v-else></fail-view>
    </div>
</template>

<script>
import fail from '../components/fail'
export default {
    data(){
        return{
            loadShow:false,
            throttle_B:false,
            scrollview:'',
            page:1,
            payAutoData:[]
        }
    },
    components:{
        'fail-view':fail
    },
    mounted(){
        this.ajaxEvent();
        this.scrollview = this.$refs.scrollview;
        this.scrollview.addEventListener('scroll',this.Pulluploading,false)
    },
    methods:{
        ajaxEvent(){
            var _self = this;
            _self.$store.state.loadShow = true;
            _self.$axios.post('https://www.yixueqm.com/cartoon/index.php/Home-User-selectAuto',_self.$qs.stringify({page:_self.page,uid:_self.$store.state.uid}))
            .then(function(response){
                if(response.data == ""){
                    _self.scrollview.removeEventListener('scroll',_self.Pulluploading,false)
                    _self.loadShow = !_self.loadShow;
                }else{
                    _self.payAutoData = _self.payAutoData.concat(response.data);
                }
                _self.$store.state.loadShow =false;
            })
        },
        setPayAutoEvent(item){
            var _self = this;
            console.log(item)
            layer.open({
                content: '是否要取消自动购买吗？'
                ,style:'max-width:70%;'
                ,btn: ['确定', '取消']
                ,yes: function(index){
                    _self.$axios.post('https://www.yixueqm.com/cartoon/index.php/Home-User-deleteAuto',_self.$qs.stringify({uid:_self.$store.state.uid,cid:item.cid}))
                    .then(function(response){
                         //提示
                        layer.open({
                            content: '操作成功'
                            ,skin: 'msg'
                            ,time: 2 //2秒后自动关闭
                            ,success(){
                                 _self.$axios.post('https://www.yixueqm.com/cartoon/index.php/Home-User-selectAuto',_self.$qs.stringify({page:1,uid:_self.$store.state.uid}))
                                .then(function(response){
                                    _self.payAutoData = response.data;
                                    _self.$store.state.loadShow =false;
                                })
                            }
                        });
                    })
                layer.close(index);
                }
            });
        },
        Pulluploading(){
            var _self = this;
            var scrollTop = _self.scrollview.scrollTop;
            var scrollHeight = _self.scrollview.scrollHeight;
            var clientHeight = _self.scrollview.clientHeight;
            if(scrollHeight - clientHeight < scrollTop+1){
                if(!_self.throttle_B){
                    _self.$store.state.loadShow =true;
                    setTimeout(function(){
                        _self.page++;
                        _self.ajaxEvent();
                        _self.throttle_B = false;
                    },300)
                    _self.throttle_B = true;
                }else{
                    return;
                }
            }
        },
    }
}
</script>

<style>
    .payAdministration_container{
        height:100vh;
        overflow: auto;
        -webkit-overflow-scrolling: touch;
    }
    .pay_list{
        padding:0.4rem 0;
        list-style: none;
    }
    .pay_item{
        padding:0 0.24rem;
        display: flex;
        margin-bottom:0.3rem;
        align-items: center;
        -webkit-tap-highlight-color: transparent;
    } 
    .pay_item:nth-last-child(1){margin-bottom: 0;}
    .pay_item_img{
        width: 1.5rem;
        height:2.1rem;
        vertical-align: middle;
        object-fit:cover;
    }
    .pay_item_c{
        margin-lefT:0.3rem;
        flex: 1;
    }
    .pay_item_c h3{
        line-height: .5rem;
        color: #333;
        font-size: .32rem;
    }
    .pay_item_c p{
        line-height: .44rem;
        color: #909090;
        font-size: .26rem;
    }
    .pay_item_c p:nth-last-child(1){margin-top:0.3rem;}
    .payautoBtn{
        -webkit-tap-highlight-color: transparent;
        width: 1.7rem;
        line-height: .46rem;
        border: .02rem solid #fd7d49;
        color: #fd7d49;
        box-sizing: border-box;
        text-align: center;
        border-radius:0.7rem;
        font-size: .28rem;
        align-self:flex-start;
    }
    .load_promt{
        line-height: .6rem;
        font-size: .28rem;
        color: #909090;
        text-align: center;
    }
</style>


